...
Theme configs are bundled with this skill at `~/projects/setup/linked/claude/skills/mermaid/`:
...
**File format:** When writing a `.md` file, always wrap the diagram in a mermaid code fence so `mmdc` can find it:
...
The bundled `mermaid-config.json` handles theme variables, padding, and spacing. Keep the init block minimal to avoid conflicts.
...
- VS Code preview: `Cmd+Shift+V` (needs "Markdown Preview Mermaid Support" extension)
