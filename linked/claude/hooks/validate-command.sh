...
if [[ "$COMMAND" =~ ^git[[:space:]]branch[[:space:]]+-[a-zA-Z]*D ]]; then
	echo "BLOCKED: git branch -D (force delete) not allowed. Use -d for merged branches."
	exit 2
fi
...
if [[ "$COMMAND" =~ ^git[[:space:]]restore[[:space:]] ]] && ! [[ "$COMMAND" =~ --staged ]]; then
	echo "BLOCKED: git restore discards working changes. Use --staged to unstage only."
	exit 2
fi
if [[ "$COMMAND" =~ ^git[[:space:]]stash[[:space:]]+(drop|clear) ]]; then
	echo "BLOCKED: git stash drop/clear can permanently lose stashed work"
	exit 2
fi
