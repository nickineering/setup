...
if [[ -z "$git_email" ]]; then
	echo -e "${yellow}⚠ git email not configured, skipping GPG setup${reset}"
	return 0
fi
...
if [[ -z "$git_name" ]]; then
	echo -e "${yellow}⚠ git name not configured, skipping GPG setup${reset}"
	return 0
fi
...
if [[ -z "$key_id" ]]; then
	echo -e "${yellow}⚠ GPG key generation may have failed${reset}"
	return 0
fi
...
if gh auth status &>/dev/null; then
	...
	gpg --armor --export "$key_id" | gh gpg-key add - 2>/dev/null &&
		...
		echo -e "${yellow}⚠ Failed to add GPG key to GitHub (add manually)${reset}"
else
	echo -e "${yellow}⚠ GitHub CLI not authenticated. Add GPG key manually:${reset}"
